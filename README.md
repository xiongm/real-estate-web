...
3. **Design envelopes:** Open `http://localhost:3000/request-sign`, select a project, and the builder will pull its investors. Upload/preview the PDF, drag fields onto the document, and assign each field to one of the project investors. Save the request as a draft to resume it later from Admin; saved drafts autosave subsequent field changes.
