...
test.describe('navigation helpers', () => {
  const orderedFields = [
    ...
    { id: 'b', type: 'checkbox', required: true, page: 1, x: 40, y: 640 },
...
test.describe('signing navigation UI', () => {
  test('Start/Next focuses required fields and updates remaining count', async ({ page }) => {
    ...
    const fields = [
      { id: 'f1', type: 'text', required: true, page: 1, x: 120, y: 620, w: 220, h: 28 },
      { id: 'f2', type: 'checkbox', required: true, page: 1, x: 120, y: 560, w: 20, h: 20 },
      { id: 'f3', type: 'text', required: true, page: 1, x: 120, y: 500, w: 220, h: 28 },
    ];
  ...
  test('Action chip stays on the left and navigates without auto-filling signatures', async ({ page }) => {
    ...
    if (position) {
      ...
      expect(position.chipRight).toBeLessThanOrEqual(position.docLeft + 220);
    }
  ...
  test('Next scrolls to field on later page', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      ...
      return rect.top >= 0 && rect.bottom <= vh + 4;
    });
  });
